...
export async function up(knex: Knex) {
	return knex
		.schema
		.createTable(ETableNames.CharacterSavingThrows, table => {
			...
			table.comment('Table from character save throws register');
		});
}

export async function down(knex:Knex) {
	return knex.schema.dropTable(ETableNames.CharacterSavingThrows);
}